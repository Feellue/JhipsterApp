...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('myappApp.tag.home.title');
  ...
  it('should load create Tag page', async () => {
    ...
    expect(await tagUpdatePage.getPageTitle()).to.eq('myappApp.tag.home.createOrEditLabel');
  ...
  it('should create and save Tags', async () => {
    ...
    await promise.all([tagUpdatePage.setNameInput('yrau')]);

    expect(await tagUpdatePage.getNameInput()).to.eq('yrau', 'Expected Name value to be equals to yrau');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('myappApp.tag.delete.question');
