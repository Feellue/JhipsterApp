...
export class SystemePaiementComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.systemePaiementService
      ...
      .subscribe(
        (res: HttpResponse<ISystemePaiement[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInSystemePaiements(): void {
    this.eventSubscriber = this.eventManager.subscribe('systemePaiementListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISystemePaiement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/systeme-paiement'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
