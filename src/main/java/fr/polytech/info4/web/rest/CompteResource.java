...
public class CompteResource {
     ...
     */
    @GetMapping("/comptes")
    public ResponseEntity<List<CompteDTO>> getAllComptes(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("restaurant-is-null".equals(filter)) {
            log.debug("REST request to get all Comptes where restaurant is null");
            return new ResponseEntity<>(compteService.findAllWhereRestaurantIsNull(),
                    HttpStatus.OK);
        }
